# ============================================================================
#            PowerShell ADB Utility Script (GitHub Version 1.2)
# ============================================================================
# This script can either uninstall or install apps on an Android device via ADB.
# v1.2: Fixed the detection logic for 'Failure [-1000]' error.
# ============================================================================
...
$InstallListFile = Join-Path -Path $PSScriptRoot -ChildPath "install_list.txt"
...
function Start-RemovalProcess {
    ...
    Get-Content $UninstallListFile | ForEach-Object {
        ...
        if ($packageName) {
            ...
            $result = ""
            $fallbackResult = "" # Ensure fallbackResult is initialized for every loop
            try {
            ...
            # --- CORRECTED: Fallback Logic Check ---
            # Using .Contains() for a literal string check, which is not confused by the brackets [].
            if ($result.Contains("Failure [-1000]")) {
